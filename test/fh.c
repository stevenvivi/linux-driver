 ...
 * $Author: Tower
 * $Date: Sat July  21  17:15:49 CST 2010
 */
...
int main(int argc, char *argv[])
{
	...
	data = mmap(NULL, fix.smem_len, PROT_READ | PROT_WRITE, MAP_SHARED, fbdev, 0); 
	save = malloc(fix.smem_len);

	memcpy(save, data, fix.smem_len);
	fbp = (struct pixel_t **)malloc(sizeof(struct pixel_t *) * var.yres);
	for (i = 0; i < var.yres; ++i)
	{
		fbp[i] = (struct pixel_t *)(save + i * var.xres * var.bits_per_pixel / 8);
	}
    ...
    /*
	printf("cinfo.output_width=%d/tcinfo.output_height=%d/tcinfo.output_components=%d/n",cinfo.output_width,cinfo.output_height,cinfo.output_components);
    printf("cinfo.output_scanline=%d/n",cinfo.output_scanline);
    */

    /*
	 * display the JPEG on 32bit/linux
	 */
